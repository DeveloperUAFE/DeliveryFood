...
function cards() {
  class MenuDescription {
    constructor(image, alt, subtitle, itemDescr, price, parentSelector) {
      this.image = image;
      this.alt = alt;
  ...
  axios.get('http://localhost:3000/menu').then(data => data.data.forEach(_ref => {
    let {
      img,
      altimg,
      title,
    ...
    new MenuDescription(img, altimg, title, descr, price, '.menu .container').render();
  }));
}
...
function modal(triggerModal, triggerButtons, modalTimerId) {
  ...
  function showModalByScroll() {
    if (scrollY >= document.documentElement.scrollHeight - document.documentElement.clientHeight - 2) {
      openModal(triggerModal, modalTimerId);
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_4__.openModal)('.modal', '.modal__dialog', modalTimerId), 50000);
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2023-05-02');
});
}();
/******/ })()
;
